 ...
 *		sub-sampled at different spatial orientationss;
...
void getHorSubImageSixTap( StorablePicture *s, int dst_y, int dst_x, int src_y, int src_x ) 
{
  ...
  for (jpad = 0; jpad < ypadded_size; jpad++)
  {
    ...
    {      
      ...
      wBufDst[ipad] = (imgpel) iClip3 (0, img->max_imgpel_value, shift_right_round( is, 5 ) );
      iBufDst[ipad] =  is;
    }
    // center
  ...
  if ( !use_stored_int ) { // causes code expansion but is better since we avoid too many
    ...
    {	  
      ...
      {      
        ...
        wxLineDst[ipad] = (imgpel) iClip3 (0, img->max_imgpel_value, shift_right_round( is, 5 ) );
      }
    }
    // center
  ...
  else 
  {
    ...
    {	
      ...
      {  
        ...
        wxLineDst[ipad] = (imgpel) iClip3 (0, img->max_imgpel_value, shift_right_round( is, 10 ) );
      }
    }
...
void getHorSubImageBiLinear( StorablePicture *s, int dst_y, int dst_x, int src_y_l, int src_x_l, int src_y_r, int src_x_r, int offset ) 
{
  ...
  for (jpad = 0; jpad < ypadded_size; jpad++)
  {
    ...
    {      
      wBufDst[ipad] = (imgpel)
        shift_right_round( wBufSrcL[ipad] + wBufSrcR[ipad + offset], 1 );
    }
    // right padded area
    for (ipad = xpadded_size_left; ipad < xpadded_size; ipad++)
    {      
      wBufDst[ipad] = (imgpel)
        shift_right_round( wBufSrcL[ipad] + wBufSrcR[maxx], 1 );
    }
  }
}
...
void getVerSubImageBiLinear( StorablePicture *s, int dst_y, int dst_x, int src_y_l, int src_x_l, int src_y_r, int src_x_r, int offset ) 
{
  ...
  for (jpad = 0; jpad < ypadded_size_top; jpad++)
  {
    ...
    {
      wBufDst[ipad] = (imgpel)
        shift_right_round(wBufSrcL[ipad] + wBufSrcR[ipad], 1);
    }
  }
  // bottom
                             ...
                             int offset_y_r, int offset_x_r ) 
{
  ...
  {      
    ...
    {	
      wBufDst[ipad] = (imgpel)
        shift_right_round(wBufSrcL[imin (maxx, ipad + offset_x_l)] + 
