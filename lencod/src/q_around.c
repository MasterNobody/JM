...
void update_offset_params(Macroblock *currMB, int mode, Boolean luma_transform_size_8x8_flag)
{
  ...
  int luma_pos = AdaptRndPos[(is_inter<<1) + luma_transform_size_8x8_flag][p_Img->type];
  int i,j;
  int qp = currMB->qp + p_Img->bitdepth_luma_qp_scale;
  ...
  int **fAdjust = luma_transform_size_8x8_flag ? p_Img->ARCofAdj8x8[0][mode] : p_Img->ARCofAdj4x4[0][mode];
  
  if (mode == IPCM) return;

  if( (p_Img->active_sps->chroma_format_idc == YUV444) && IS_INDEPENDENT(p_Inp) )
  {
    if( luma_transform_size_8x8_flag )  // 8x8
      luma_pos += 5 * p_Img->colour_plane_id;
    else  // 4x4
      luma_pos += p_Img->colour_plane_id;
  }
  ...
  { 
    ...
    for(uv = 0; uv < 2; uv++)
    {
      luma_pos = AdaptRndPos[(is_inter<<1) + luma_transform_size_8x8_flag][p_Img->type];
      fAdjustCbCr = luma_transform_size_8x8_flag ? p_Img->ARCofAdj8x8[uv + 1][mode] : p_Img->ARCofAdj4x4[uv + 1][mode];
      if(luma_transform_size_8x8_flag )  // 8x8
        luma_pos += 5 * (uv+1);
      else  // 4x4
        luma_pos += (uv+1);
  ...
  if ((p_Inp->AdaptRndChroma) && (p_Img->yuv_format == YUV420 || p_Img->yuv_format == YUV422 ))
  {  
    int temp_offset;
    int u_pos = AdaptRndCrPos[is_inter][p_Img->type];
    int v_pos = u_pos + 1;
    int jpos;

    //int ***fAdjustCr = is_inter ? bestOffset.InterFAdjust4x4Cr : bestOffset.IntraFAdjust4x4Cr;

    int **fAdjustCb = (luma_transform_size_8x8_flag && mode == P8x8 )? p_Img->ARCofAdj4x4[1][4] : p_Img->ARCofAdj4x4[1][mode];
    int **fAdjustCr = (luma_transform_size_8x8_flag && mode == P8x8 )? p_Img->ARCofAdj4x4[2][4] : p_Img->ARCofAdj4x4[2][mode];
...
{
  int maxplane;
  maxplane = (p_Img->yuv_format == YUV400)?  1 : 3;
  memset(&p_Img->ARCofAdj4x4[0][0][0][0], 0, maxplane * MAXMODE * MB_PIXELS * sizeof (int));
  maxplane = (p_Img->yuv_format == YUV400)?  1 : (p_Img->P444_joined ? 3 : 1);
  memset(&p_Img->ARCofAdj8x8[0][0][0][0], 0, maxplane * MAXMODE * MB_PIXELS * sizeof (int));
}
...
void reset_adaptive_rounding_direct(ImageParameters *p_Img)
{
  ...
  for (pl = 0; pl < maxplane; pl++)
    memset(&p_Img->ARCofAdj4x4[pl][DUMMY][0][0], 0, MB_PIXELS * sizeof (int));

  maxplane = (p_Img->yuv_format == YUV400)?  1 : (p_Img->P444_joined ? 3 : 1);
  for (pl = 0; pl < maxplane; pl++)
    memset(&p_Img->ARCofAdj8x8[pl][DUMMY][0][0], 0, MB_PIXELS * sizeof (int));
