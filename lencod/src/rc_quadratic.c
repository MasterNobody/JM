...
void rc_update_pict_frame(rc_quadratic *prc, int nbits)
{
  ...
  /* frame layer rate control */
  if(img->BasicUnit==img->FrameSizeInMbs)
    complexity = (int) floor(nbits * prc->m_Qc + 0.5);
  /* basic unit layer rate control */
  else
  {
    if( (img->type == P_SLICE || input->RCUpdateMode == RC_MODE_1) && (IMG_NUMBER) )
    {
      if( (generic_RC->NoGranularFieldRC == 0 && generic_RC->FieldControl == 1) || generic_RC->FieldControl == 0 )
      {
        ...
        complexity = (int)floor(nbits*Avem_Qc+0.5);
      }
    }
    else if( img->type == B_SLICE )
      complexity = (int) floor(nbits*prc->m_Qc+ 0.5);
  ...
  if (img->type == I_SLICE && IMG_NUMBER && input->RCUpdateMode == RC_MODE_3)
    generic_RC->NIslice--;
  ...
  if ( (img->type == P_SLICE || input->RCUpdateMode == RC_MODE_1) && (IMG_NUMBER) )
  {
    ...
    else if((generic_RC->NoGranularFieldRC!=0)&&(generic_RC->FieldControl==1))
      generic_RC->NoGranularFieldRC=0;
  }
  else if ( img->type == B_SLICE )
  {
  ...
  if(img->BasicUnit==img->FrameSizeInMbs || img->type != P_SLICE )
  {
    ...
    {      
      ...
      else
      {
        ...
        if ( img->BasicUnit < img->FrameSizeInMbs && img->type != P_SLICE )
        {
          // when RC_MODE_3 or RC_MODE_4 is set and basic unit is smaller than a frame, note that:
 ...
 *************************************************************************************
*/
int updateQPRC4(rc_quadratic *prc, int topfield)
{
  int m_Bits;
  int SumofBasicUnit;
  int DuantQp, m_Qp, m_Hp; 
