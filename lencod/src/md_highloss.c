...
void encode_one_macroblock_highloss (Macroblock *currMB)
{
  ...
  int         cost=0;
  int         min_cost = INT_MAX, cost_direct=0;
  int         intra1 = 0;
  int         mb_available[3];
  ...
  int         lambda_mf[3];
  short       runs        = (short) ((p_Inp->RestrictRef==1 && (pslice  || (bslice && p_Img->nal_reference_idc>0))) ? 2 : 1);
  ...
  for (rerun=0; rerun<runs; rerun++)
  {
    ...
    if (enc_mb.valid[0])
      ...
      if (enc_mb.valid[P8x8])
      {
        ...
        // following p_Inp could be added in RD_8x8DATA structure        
        }// if (p_Inp->Transform8x8Mode)
