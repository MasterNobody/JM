 ...
 ************************************************************************
 */
void OpenTiffFile( VideoDataFile *input_file, InputParameters *params, int FrameNumberInFile)
{
 ...
 ************************************************************************
 */
static void ReadTIFFIFDEntry (int vfile, TIFFHeader *tiffHeader, unsigned short *ifd_count, TIFFIFDEntry **tiffIFD)
{
  int i;
  ...
  if (read(vfile, ifd_count, sizeof(unsigned short)) != sizeof(unsigned short))
  {
 ...
 ************************************************************************
 */
static void ParseTIFFIFD (unsigned short ifd_count, TIFFIFDEntry *tiffIFD, FrameFormat *source)
{
  int i;
  ...
  for (i = 0; i < ifd_count; i++)
  {
    ...
    case TIFFTAG_IMAGEWIDTH : 
      if (tiffIFD[i].tIFD_offset != source->width)
      {
    ...
    case TIFFTAG_IMAGELENGTH :
      if (tiffIFD[i].tIFD_offset != source->height)
      {
