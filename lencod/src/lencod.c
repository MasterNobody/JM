...
void main(int argc,char **argv)
{
  ...
  for (img->number=0; img->number < inp->no_frames; img->number++) 
  {
    ...
    */   
    ...
    find_snr(snr,img,inp);        /* find SNR for the last image */ 
  ...
  sprintf(tracestring, "-------- End of seq ---- " );
  put(tracestring,LEN_STARTCODE,EOS);
  ...
  fprintf(stdout,"--------------------------------------------------------------------------\n");    
  fprintf(stdout,"Exit TML 4 encoder ver 4.3\n");            
...
void read_input(struct inp_par *inp,char config_filename[])
{
  ...
  for (i=0;i<8;i++)
    ...
    if ((p_trace=fopen("trace.txt","wb"))==0)
    {
      printf("Error open trace file trace.txt \n");
    }
}
...
*                    
************************************************************************/
void init(struct img_par *img,struct stat_par *stat)
{
  const int SPIRAL0[9][2]=
  {
