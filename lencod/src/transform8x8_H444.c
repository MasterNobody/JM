  ...
  *min_cost = INT_MAX;
  ...
  for (ipmode = 0; ipmode < NO_INTRA_PMODE; ipmode++)
  {
      ...
      (all_available) )
    {
      ...
      currSlice->compute_cost8x8(p_Img, &p_Img->pImgOrg[0][pic_opix_y], currSlice->mpr_8x8[0][ipmode], pic_opix_x, &cost, (int) *min_cost);
     
      if(currSlice->P444_joined)
      {
        get_intrapred_8x8(currMB, PLANE_U, ipmode, left_available, up_available);
        currSlice->compute_cost8x8(p_Img, &p_Img->pImgOrg[PLANE_U][pic_opix_y], currSlice->mpr_8x8[PLANE_U][ipmode], pic_opix_x, &cost, (int) *min_cost);
        get_intrapred_8x8(currMB, PLANE_V, ipmode, left_available, up_available);
        currSlice->compute_cost8x8(p_Img, &p_Img->pImgOrg[PLANE_V][pic_opix_y], currSlice->mpr_8x8[PLANE_V][ipmode], pic_opix_x, &cost, (int) *min_cost);
      }
  ...
  {
    ColorPlane k;
    p_Img->CbCr_predmode_8x8[b8] = best_ipmode; 
    for (k = PLANE_U; k <= PLANE_V; k++)
    {
      ...
      for (j=0; j<8; j++)
      {
        for (i=0; i<8; i++)
        {
          ...
          currSlice->mb_ores[k][block_y+j][block_x+i] = p_Img->pImgOrg[k][currMB->pix_y + block_y + j][currMB->pix_x + block_x + i] - currSlice->mpr_8x8[k][best_ipmode][j][i];
        }
      }
  ...
  *min_cost = INT_MAX;
  ...
  for (ipmode = 0; ipmode < NO_INTRA_PMODE; ipmode++)
  {
      ...
      (all_available) )
    {
      ...
      // get prediction and prediction error
      generate_pred_error_8x8(&p_Img->pImgOrg[0][pic_opix_y], currSlice->mpr_8x8[0][ipmode], &currSlice->mb_pred[0][block_y], &currSlice->mb_ores[0][block_y], pic_opix_x, block_x);

      if(currSlice->P444_joined) 
      {
        generate_pred_error_8x8(&p_Img->pImgOrg[1][pic_opix_y], currSlice->mpr_8x8[1][ipmode], &currSlice->mb_pred[1][block_y], &currSlice->mb_ores[1][block_y], pic_opix_x, block_x);
        generate_pred_error_8x8(&p_Img->pImgOrg[2][pic_opix_y], currSlice->mpr_8x8[2][ipmode], &currSlice->mb_pred[2][block_y], &currSlice->mb_ores[2][block_y], pic_opix_x, block_x);
      }
      ...
      if ((rdcost = currSlice->rdcost_for_8x8_intra_blocks (currMB, &c_nz, b8, ipmode, lambda, min_rdcost, mostProbableMode)) < min_rdcost)
      {
        ...
        if (p_Img->AdaptiveRounding)
        {
          ...
          if (currSlice->P444_joined)
          {
            for (j = block_y; j < block_y + 8; j++)
            {
              memcpy(&p_Img->ARCofAdj8x8[1][DUMMY][j][block_x],&p_Img->ARCofAdj8x8[1][I8MB][j][block_x], 8 * sizeof(int));
              memcpy(&p_Img->ARCofAdj8x8[2][DUMMY][j][block_x],&p_Img->ARCofAdj8x8[2][I8MB][j][block_x], 8 * sizeof(int));
            }
          }            
        }
        ...
        { 
          ...
          for (uv=0; uv < 2; uv++)
          {
            ...
            //--- set reconstruction ---
            copy_image_data_8x8(p_RDO->rec8x8[uv + 1], &p_Img->enc_picture->imgUV[uv][pic_pix_y], 0, pic_pix_x);
          }
        }
  ...
  if (p_Img->AdaptiveRounding)
  {
    ...
    if (currSlice->P444_joined)
    {
      for (j=0; j<8; j++)
      {
        memcpy(&p_Img->ARCofAdj8x8[1][I8MB][block_y + j][block_x], &p_Img->ARCofAdj8x8[1][DUMMY][block_y + j][block_x], 8 * sizeof(int));
        memcpy(&p_Img->ARCofAdj8x8[2][I8MB][block_y + j][block_x], &p_Img->ARCofAdj8x8[2][DUMMY][block_y + j][block_x], 8 * sizeof(int));
      }
    }
  }
  ...
  if (currSlice->P444_joined)
  {
    ...
    //===== restore reconstruction and prediction (needed if single coeffs are removed) =====
    copy_image_data_8x8(&p_Img->enc_picture->imgUV[0][pic_pix_y], p_RDO->rec8x8[1], pic_pix_x, 0);
    copy_image_data_8x8(&p_Img->enc_picture->imgUV[1][pic_pix_y], p_RDO->rec8x8[2], pic_pix_x, 0);
